...
Controls:
    ...
    pivot slider   — ppm position that stays fixed when sweeping p1 (red dashed line)
...
def apply_phase(fft_data, p0, p1, pivot_idx):
    """Phase correct with a pivot: phase(i) = p0 + p1*(i - pivot_idx)/N."""
    npts = len(fft_data)
...
def main():
    ...
    # ── Buttons ──────────────────────────────────────────────────────────────
    ax_btn_zoom  = fig.add_axes([0.20, 0.01, 0.25, 0.04])
    ax_btn_print = fig.add_axes([0.55, 0.01, 0.25, 0.04])
    ...
    def redraw(p0, p1, pivot_ppm):
        ...
        title.set_text(f"p0 = {p0:.2f}°   p1 = {p1:.2f}°   pivot = {pivot_ppm:.2f} ppm")
    ...
    def on_print(_):
        ...
        pivot_idx = int(np.argmin(np.abs(ppm - pivot_ppm)))
        # Convert to ng.proc_base.ps convention (pivot at index 0)
        p0_yaml = p0 - p1 * pivot_idx / len(fft_data)
        print(f"\n--- Copy into your YAML ---")
        print(f"    phase: \"manual\"")
